...
def prepare_cifar100():
	...
	train_set = datasets.CIFAR100("../data", download=True, transform=
													tf.Compose([tf.ToTensor(),normalize]))
	test_set = datasets.CIFAR100("../data", download=True, train=False, transform=
												   tf.Compose([tf.ToTensor(),normalize])) 
...
def calc_metrics(model, loader):
    ...
    for i, (samples, labels) in enumerate(loader):
        ...
        y_true.append(labels.data.view_as(predicted))
